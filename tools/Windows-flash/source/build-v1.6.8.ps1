...
$version = if ($Release) { '1.6.8' } else { "1.6.8-$CandidateId" }
...
if ($approved.version -cne '1.4.5') { throw '1.6.8只接受1.4.5批准合同，候选同样不得回退旧固件' }
...
if ($basic.versionCode -ne 193 -or $basic.size -ne 135645 -or $full.versionCode -ne 194) { throw '1.6.8必须绑定基础193及完整194，基础包大小必须为135645字节' }
...
& (Join-Path $projectRoot 'tests/BasicProbeIntegratedTests.ps1') -Executable $candidate -MetadataPath $MetadataPath -OutputDirectory (Join-Path $reportDirectory 'basic-probe-exe') -ExpectedToolVersion '1.6.8'
