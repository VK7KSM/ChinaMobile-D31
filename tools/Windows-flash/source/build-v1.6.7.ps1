...
$version = if ($Release) { '1.6.7' } else { "1.6.7-$CandidateId" }
...
$firmwareContract = Read-ApprovedFirmwareContract $approvedPath $InstalledFilesPath -Release:$Release
...
& (Join-Path $projectRoot 'tests/BasicProbeIntegratedTests.ps1') -Executable $candidate -MetadataPath $MetadataPath -OutputDirectory (Join-Path $reportDirectory 'basic-probe-exe')
