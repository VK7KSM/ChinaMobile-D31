param(
    ...
    [string]$AaptPath = 'C:/Dev/android-sdk/build-tools/34.0.0/aapt.exe',
    [string]$ApkSignerJar = 'C:/Dev/android-sdk/build-tools/34.0.0/lib/apksigner.jar',
    [string]$JavaPath = '',
    [string]$BashPath = 'C:/Program Files/Git/bin/bash.exe'
)
$ErrorActionPreference = "Stop"
if (-not $Release -and -not $CandidateId) { throw '必须选择正式构建或明确候选编号' }

$version = if ($Release) { '1.6.11' } else { "1.6.11-$CandidateId" }
$projectRoot = Split-Path -Parent $MyInvocation.MyCommand.Path
$sourceDirectory = Join-Path $projectRoot "src"
$outputDirectory = Join-Path $projectRoot "dist-v$version"
$objectDirectory = Join-Path $projectRoot "obj-v$version"
...
if ($Release -and (-not $ApprovedPackagePath -or -not $InstalledFilesPath -or -not $FirmwarePackage)) { throw '正式构建必须明确指定已批准固件、批准合同和安装清单' }
if (-not $ApprovedPackagePath) { $ApprovedPackagePath = Join-Path $backendRoot 'approved-package.json' }
$approvedPath = (Resolve-Path -LiteralPath $ApprovedPackagePath).Path
Import-Module (Join-Path $projectRoot 'tools/FirmwareContract.psm1') -Force
$firmwareContract = Read-ApprovedFirmwareContract $approvedPath $InstalledFilesPath -Release:$Release -ExpectedReleaseVersion '1.4.6'
$approved = $firmwareContract.Approval
if ($approved.version -cne '1.4.6' -or -not $firmwareContract.ReplaceBootRecovery) { throw '1.6.11只接受1.4.6批准合同，候选同样不得回退旧固件' }
if (-not $FirmwarePackage) { $FirmwarePackage = Join-Path $projectRoot ("..\d31\dist\factory-flash-v"+$approved.version+'\'+$approved.fileName) }
$package = (Resolve-Path -LiteralPath $FirmwarePackage).Path
...
if (-not $JavaPath) { $JavaPath = Join-Path $projectRoot '..\..\.tools\jdk17\jdk-17.0.20+8\bin\java.exe' }
$BasicApk = (Resolve-Path -LiteralPath $BasicApk).Path
$FullApk = (Resolve-Path -LiteralPath $FullApk).Path
...
$toolArguments = @{ AaptPath=$AaptPath; JavaPath=$JavaPath; ApkSignerJar=$ApkSignerJar }
$basic = Test-ProbeApk $BasicApk $contract basic @toolArguments
$full = Test-ProbeApk $FullApk $contract full @toolArguments
if ($basic.versionCode -ne 193 -or $basic.size -ne 135645 -or $full.versionCode -ne 194) { throw '1.6.11必须绑定基础193及完整194，基础包大小必须为135645字节' }
if ($approved.elfRemote -and ($full.versionCode -ne $approved.elfRemote.versionCode -or
    $full.versionName -cne $approved.elfRemote.versionName -or $full.sha256 -ne $approved.elfRemote.sha256)) { throw '完整APK与系统批准合同不一致' }
$nativePayload = if ($approved.elfRemote) { Assert-FirmwareNativePayload $FullApk $InstalledFilesPath } else { $null }
...
foreach ($directory in @($outputDirectory,$objectDirectory)) {
    if (Test-Path -LiteralPath $directory) { throw "候选目录已存在，拒绝覆盖：$directory" }
}

New-Item -ItemType Directory -Force -Path $objectDirectory, $runtimeStage, $reportDirectory | Out-Null
$prepared = & (Join-Path $projectRoot 'tools/Prepare-BasicProbeResources.ps1') -BasicApk $BasicApk -MetadataPath $MetadataPath -OutputDirectory (Join-Path $objectDirectory 'basic-resource') @toolArguments
$basicResource = Get-Content -LiteralPath $prepared.Manifest -Raw -Encoding UTF8 | ConvertFrom-Json
Assert-ProbeResourceManifest $basicResource $basic
$basicSource = Join-Path $prepared.StageDirectory $basicResource.runtimeSources[0].Source
...
if ($InstalledFilesPath) {
    Copy-Item -LiteralPath $InstalledFilesPath -Destination $installedManifest
} else {
if ($approved.version -cne '1.4.3') { throw '新固件必须提供独立批准安装清单' }
...
$runtimeSources = @(
    ...
    [PSCustomObject]@{ Relative = $basicResource.runtimeSources[0].Relative; Source = $basicSource; Bom = $false },
...
for ($index = 0; $index -lt $runtimeSources.Count; $index++) {
    ...
    if ($asset.Relative -ceq $basicResource.runtimeFiles[0].RelativePath) { $resourceName = $basicResource.runtimeFiles[0].ResourceName }
...
# 只加载已嵌入后端的纯清单检查函数，不执行其ADB主流程。
$tokens=$null; $parseErrors=$null
$backendAst=[System.Management.Automation.Language.Parser]::ParseFile((Join-Path $runtimeStage 'flash_d31_recovery.ps1'),[ref]$tokens,[ref]$parseErrors)
if ($parseErrors.Count) { throw '内置刷机后端语法错误' }
$manifestCheck=$backendAst.Find({param($node) $node -is [System.Management.Automation.Language.FunctionDefinitionAst] -and $node.Name -eq 'Read-InstalledPayloadManifest'},$true)
if (-not $manifestCheck) { throw '缺少内置安装清单检查' }
& { param($PackageRoot,$ApprovedPackage,$Definition); Invoke-Expression $Definition; $null=Read-InstalledPayloadManifest } $runtimeStage $approved $manifestCheck.Extent.Text
...
namespace D31FlashTool
{
    internal static class BuildConstants
    {
        ...
        internal const string OfficialPackageName = "$($approved.fileName)";
        internal const string OfficialGitHubUrl = "$($firmwareContract.GitHubUrl)";
        internal const string OfficialCloudflareUrl = "$($firmwareContract.CloudflareUrl)";
...
$sources = @(Get-ChildItem -LiteralPath $sourceDirectory -Filter "*.cs" -File | Where-Object Name -NE 'BuildConstants.g.cs' | ForEach-Object { $_.FullName })
$sources += $generated
$compilerArguments = @(
    "/nologo",
    "/target:winexe",
...
$portTest = Join-Path $objectDirectory 'AdbPortTests.exe'
& $csc /nologo /reference:System.Web.Extensions.dll /reference:System.Drawing.dll /reference:System.Windows.Forms.dll /main:AdbPortTests "/out:$portTest" @sources (Join-Path $projectRoot 'tests/AdbPortTests.cs')
if ($LASTEXITCODE -ne 0) { throw 'ADB端口测试编译失败' }
& $portTest (Join-Path $reportDirectory 'ADB端口测试.txt') $BashPath (Join-Path $projectRoot 'tests/adb_port_fixture.sh')
if ($LASTEXITCODE -ne 0) { throw 'ADB端口测试失败' }
& (Join-Path $projectRoot 'tests/TestPlatformCompatibility.ps1') -OutputDirectory (Join-Path $reportDirectory 'platform-compatibility') -ConstantsPath $generated
& (Join-Path $projectRoot 'tests/TestFirmware1611Contract.ps1') -OutputDirectory (Join-Path $reportDirectory 'firmware-contract')
& (Join-Path $projectRoot 'tests/TestFlashWorkflow.ps1') -OutputDirectory (Join-Path $reportDirectory 'flash-workflow') -ConstantsPath $generated
$maintenanceTest = Join-Path $objectDirectory 'BasicProbeMaintenanceTests.exe'
& $csc /nologo "/out:$maintenanceTest" (Join-Path $sourceDirectory 'MaintenanceGuard.cs') (Join-Path $projectRoot 'tests/BasicProbeMaintenanceTests.cs')
if ($LASTEXITCODE -ne 0) { throw '维护标记测试编译失败' }
& $maintenanceTest (Join-Path $reportDirectory 'maintenance') $BashPath (Join-Path $projectRoot 'tests/BasicProbeMaintenanceFixture.sh')
if ($LASTEXITCODE -ne 0) { throw '维护标记测试失败' }
...
$process = Start-Process -FilePath $candidate -ArgumentList @('--self-test',('"'+$selfTestReport+'"'),('"'+$package+'"')) -WindowStyle Hidden -Wait -PassThru
if ($process.ExitCode -ne 0) {
    throw ("D31刷机工具离线自检失败：" + [Environment]::NewLine + (Get-Content -Raw $selfTestReport))
}
$verify = Start-Process -FilePath $candidate -ArgumentList @('--verify-package',('"'+$package+'"'),('"'+$packageReport+'"')) -WindowStyle Hidden -Wait -PassThru
if ($verify.ExitCode -ne 0) {
    throw ("D31刷机包选择流程测试失败：" + [Environment]::NewLine + (Get-Content -Raw $packageReport))
}
$downloadParser = Start-Process -FilePath $candidate -ArgumentList @('--test-download-progress',('"'+$downloadParserReport+'"')) -WindowStyle Hidden -Wait -PassThru
if ($downloadParser.ExitCode -ne 0) {
    throw ("高速下载进度解析测试失败：" + [Environment]::NewLine + (Get-Content -Raw $downloadParserReport))
}

& (Join-Path $projectRoot 'tests/BasicProbeTests.ps1') -BasicApk $BasicApk -FullApk $FullApk -MetadataPath $MetadataPath -OutputDirectory (Join-Path $reportDirectory 'basic-probe-tools') @toolArguments
& (Join-Path $projectRoot 'tests/BasicProbeIntegratedTests.ps1') -Executable $candidate -MetadataPath $MetadataPath -OutputDirectory (Join-Path $reportDirectory 'basic-probe-exe') -ExpectedToolVersion '1.6.11'
& (Join-Path $projectRoot 'tests/TestFlashMaintenance.ps1') -OutputDirectory (Join-Path $reportDirectory 'flash-maintenance')
if ($LASTEXITCODE -ne 0) { throw '维护部署证据测试失败' }
& (Join-Path $projectRoot 'tests/TestMaintenanceShell.ps1') -OutputDirectory (Join-Path $reportDirectory 'maintenance-shell') -BashPath $BashPath
if ($LASTEXITCODE -ne 0) { throw '维护shell证据测试失败' }
& (Join-Path $projectRoot 'tests/TestSystemFirmware.ps1') -OutputDirectory (Join-Path $reportDirectory 'system-firmware')
if ($LASTEXITCODE -ne 0) { throw '系统固件合同测试失败' }
if ($approved.elfRemote) {
    & (Join-Path $projectRoot 'tests/TestFirmwareNative.ps1') -FullApk $FullApk -InstalledFilesPath $installedManifest -OutputDirectory (Join-Path $reportDirectory 'firmware-native')
    if ($LASTEXITCODE -ne 0) { throw '完整APK系统原生库绑定测试失败' }
}
...
if ($distFiles.Count -ne 1 -or $distFiles[0].Name -ne "D31-Flash-Tool-v$version.exe") {
    throw "单文件候选门失败：dist目录并非只含一个EXE"
}
...
$result = [PSCustomObject]@{
    ...
    基础探针版本 = $basic.versionCode
    基础探针SHA256 = $basic.sha256
    完整候选版本 = $full.versionCode
    完整候选SHA256 = $full.sha256
    制品索引SHA256 = (Get-FileHash -LiteralPath $MetadataPath).Hash
    状态 = '本地可运行候选，未发布，未进行设备操作'
    发布目录文件数 = $distFiles.Count
    内置运行文件数 = $runtimeSources.Count
    验证用刷机包字节数 = $packageItem.Length
    验证用刷机包SHA256 = $packageHash
    绑定固件版本 = $approved.version
    绑定固件文件名 = $approved.fileName
    固件批准合同SHA256 = (Get-FileHash -LiteralPath $approvedPath).Hash
    安装清单SHA256 = (Get-FileHash -LiteralPath $installedManifest).Hash
    GitHub下载地址 = $firmwareContract.GitHubUrl
    Cloudflare下载地址 = $firmwareContract.CloudflareUrl
    系统原生库 = $nativePayload
