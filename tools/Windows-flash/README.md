...
当前入口为`source/build.ps1`和`source/build-v1.6.11.ps1`，配套1.4.6完整刷机包。继续使用单个EXE，内置基础193，保留“开始刷机”自动完成检查、备份及刷写的一键流程。

新流程自动保存boot/Recovery原像，必要时先将Recovery替换为包内配套环境，再安装系统和boot。不要求旧启动镜像摘要一致；只读检查不会执行这些写入。分区写入标记、失败中止和回读核验见[完整刷机说明](../../docs/D31-v1.4.6完整刷机说明.md)。以下旧版本段落和来源清单保留作历史记录。

## 1.6.10流程修正

2026年9月26日恢复“一键开始后自动检查、可选备份、刷机”的流程，“只读检查”不再是手动前置条件。同时修复Windows PowerShell 5.1剥离双引号导致的分区查询错误，并补足后端启动后日志异常时的忙碌状态处理。配套固件仍为1.4.5，基础APK仍为193。当前构建入口为`source/build.ps1`及`source/build-v1.6.10.ps1`，最终摘要见`source/release-v1.6.10.json`，来源见`source/sources-v1.6.10.json`。下面章节保留历史记录。

详细原因、测试与边界见[1.6.10说明](../../docs/D31-Windows工具1.6.10兼容性说明.md)。

## 1.6.9兼容性更新（历史）

2026年9月26日更新设备识别、备份和刷前准入：当前系统构建指纹不同仅提示，产品平台、分区映射及容量、boot/Recovery兼容性继续检查。固件仍为1.4.5，内置基础APK仍为193，交付仍为单EXE。

当前入口为`source/build.ps1`及`source/build-v1.6.9.ps1`；实际生成常量同步到`source/src/BuildConstants.g.cs`，来源摘要见`source/sources-v1.6.9.json`。此前版本的来源清单保留为历史记录，不代表当前可变源码的摘要。下面1.6.8和1.6.7章节也仅记录历史交付。

本次构建曾被集成测试的旧版本白名单中断，修正测试白名单后续跑原构建器剩余步骤；不修改受验EXE。不同构建的完整模拟流程、错误平台及分区拒绝、本机备份准入均离线验证，未进行真实刷机。详见[兼容性说明](../../docs/D31-Windows工具1.6.9兼容性说明.md)。
