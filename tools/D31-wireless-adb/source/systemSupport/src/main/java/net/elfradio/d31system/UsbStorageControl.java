...
final class UsbStorageControl {
    ...
    static SystemActions.ActionResult ensureMappingFromIntent(Context context, Intent intent) {
        String uuid = uuidFromIntent(intent);
        return ensureMapping(uuid);
    }

    static synchronized SystemActions.ActionResult refreshReadyMappings() {
        StringBuilder log = new StringBuilder();
        boolean succeeded = true;
        for (String uuid : findMountedUuids()) {
            SystemActions.ActionResult result = ensureMapping(uuid);
            log.append(result.log);
            succeeded &= result.succeeded;
        }
        return new SystemActions.ActionResult(log.toString(), succeeded);
    }

    static synchronized SystemActions.ActionResult refreshAllMappings() {
        List<String> uuids = findMountedUuids();
        SystemActions.ActionResult legacy = SystemActions.executeRoot(
                "移除旧版USB存储入口", buildRemoveLegacyCommand());
        if (!legacy.succeeded) return legacy;
        if (uuids.isEmpty()) {
            return new SystemActions.ActionResult("当前没有已挂载的USB存储卷。\n", true);
        }
        StringBuilder log = new StringBuilder(legacy.log);
        for (String uuid : uuids) {
            SystemActions.ActionResult result = ensureMapping(uuid);
            log.append(result.log);
            if (!result.succeeded) return new SystemActions.ActionResult(log.toString(), false);
        }
        return new SystemActions.ActionResult(log.toString(), true);
    }

    static synchronized SystemActions.ActionResult ensureMapping(String uuid) {
        if (!isSafeUuid(uuid)) {
            return new SystemActions.ActionResult("没有找到可用的USB存储卷。\n", false);
        }
        ...
        if (sourceDevice == null) {
            return new SystemActions.ActionResult("外置存储在处理期间已移除。\n", false);
        }
        ...
        if (sourceDevice.equals(targetDevice)) {
            return new SystemActions.ActionResult("映射已经存在且设备号匹配。\n映射回读=通过\n", true);
        }

        if (targetDevice != null) {
            SystemActions.ActionResult unmount = SystemActions.executeRoot(
        ...
        SystemActions.ActionResult steps = SystemActions.executeRootSequence(
        ...
        return new SystemActions.ActionResult(log
                + "映射回读=" + (verified ? "通过" : "失败") + "\n",
                commandsSucceeded && verified);
    }

    static synchronized SystemActions.ActionResult removeMapping(String uuid) {
        if (!isSafeUuid(uuid)) return removeAllMappings();
        if (targetDevice(uuid) == null) {
            return new SystemActions.ActionResult("USB存储入口已不存在，无需重复清理。\n", true);
        }
        return SystemActions.executeRoot("移除USB存储入口", buildRemoveCommand(uuid));
    }

    static synchronized SystemActions.ActionResult removeAllMappings() {
        return SystemActions.executeRoot("移除全部USB存储入口", buildRemoveAllCommand());
    }

    static SystemActions.ActionResult disableVendorUsbPrompt() {
        return SystemActions.executeRoot("禁用原厂U盘重复弹窗", buildDisableVendorPromptCommand());
    }
