...
public final class UsbInsertPromptActivity extends Activity {
    static final String EXTRA_VOLUME_UUID = "net.elfradio.d31bootstrap.extra.VOLUME_UUID";
    ...
    private void wakeAndDismissScreenSaver() {
        try {
            ...
            sendBroadcast(wake);
            ProbeLog.append(this, "已调用原厂屏保退出入口");
        } catch (Throwable error) {
            ProbeLog.append(this, "退出原厂屏保失败：" + error);
        }
        ...
        try {
            ...
            ProbeLog.append(this, "点亮U盘确认界面失败：" + error);
        }
    }
    ...
    private void preparePrompt(Intent intent) {
        ...
        new Thread(() -> {
            ...
            AdbControl.ActionResult result = UsbStorageControl.ensureMapping(mountedUuid);
            ProbeLog.append(this, "U盘确认对话框映射复核，卷=" + mountedUuid
                    + "，成功=" + result.succeeded
                    + "\n" + result.log);
    ...
    private void showPrompt() {
        ...
        dialog = new AlertDialog.Builder(this)
                ...
                .setPositiveButton("打开", (ignored, which) -> {
                    ...
                    new Thread(() -> {
                        AdbControl.ActionResult result = UsbStorageControl.ensureMapping(selectedUuid);
                        ProbeLog.append(this, "打开前再次复核存储卷，卷=" + selectedUuid
                                + "，成功=" + result.succeeded + "\n" + result.log);
                        runOnUiThread(() -> {
                            if (result.succeeded) {
