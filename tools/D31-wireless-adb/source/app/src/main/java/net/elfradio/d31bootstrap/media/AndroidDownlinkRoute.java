...
public final class AndroidDownlinkRoute {
    public interface Focus { void owned(boolean owned); }
    public static DownlinkRouteLease create(Context context,PttOutputGuard idle,Runnable lost){
        return create(context,idle,idle::focusOwned,lost);
    }
    /** 通话复用同一路由与恢复日志，仅注入双向空闲门和焦点观察者。 */
    public static DownlinkRouteLease create(Context context,AudioGuard idle,Focus focus,Runnable lost){
        AudioManager audio=(AudioManager)context.getSystemService(Context.AUDIO_SERVICE);
        SharedPreferences preferences=context.getSharedPreferences("ptt-route-recovery",Context.MODE_PRIVATE);
        Object focusLock=new Object();boolean[] focusLost={false};
        AudioManager.OnAudioFocusChangeListener listener=change->{if(change<=0){
            synchronized(focusLock){focusLost[0]=true;focus.owned(false);}
            lost.run();
        }};
        return new DownlinkRouteLease(new DownlinkRouteLease.Port(){
            ...
            public void speaker(boolean value)throws IOException{
                // 只保护轻量状态，不持锁调用AudioManager或上层停止回调。
                synchronized(focusLock){if(value&&focusLost[0])throw new IOException("MEDIA_PTT_FOCUS_LOST");}
                audio.setSpeakerphoneOn(value);
            }
            public boolean focus(){boolean granted=audio.requestAudioFocus(listener,AudioManager.STREAM_MUSIC,
                    AudioManager.AUDIOFOCUS_GAIN_TRANSIENT)==AudioManager.AUDIOFOCUS_REQUEST_GRANTED;
                synchronized(focusLock){focus.owned(granted&&!focusLost[0]);}
                return granted;
            }
