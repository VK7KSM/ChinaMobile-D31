...
public final class PttSessionController implements AutoCloseable {
    ...
    private final ExecutorService worker=Executors.newSingleThreadExecutor(r->new Thread(r,"d31-ptt-session"));
    ...
    public void receive(String raw){
        ...
        catch(Exception invalid){stop("MEDIA_PTT_MESSAGE_INVALID");}if(protocol.stopping())cancel();wake();
    }
    public void ice(boolean connected){protocol.ice(connected);if(protocol.stopping())cancel();wake();}
    public void tick(){protocol.tick();if(protocol.stopping())cancel();wake();}
    public void mediaChanged(){wake();}
    public JSONObject snapshot()throws Exception{return protocol.snapshot();}
    public void stop(String reason){protocol.stop(reason);cancel();wake();}
    private void cancel(){
        // 撤销只是尽力打断；失败不能跳过CLOSE，释放结论仍由close取得。
        ...
        try{
            do{
                ...
                while((action=protocol.poll())!=null){
                    ...
                    }catch(Exception|LinkageError error){protocol.stop(code(error,"MEDIA_PTT_ACTION_FAILED"));cancel();}
                }
                ...
                if("muted_verification".equals(state)||"streaming".equals(state)){
                    ...
                    }catch(Exception|LinkageError failure){protocol.stop(code(failure,"MEDIA_PTT_OUTPUT_UNKNOWN"));cancel();}
                    // 上述回调可能产生UNMUTE、ready或CLOSE，不丢弃其排队动作。
                    if(protocol.hasActions())dirty.set(true);
                }
                publish();
            }while(dirty.get()&&finished.getCount()>0);
        }catch(Exception unexpected){protocol.stop("MEDIA_PTT_CONTROLLER_FAILED");cancel();dirty.set(true);}
