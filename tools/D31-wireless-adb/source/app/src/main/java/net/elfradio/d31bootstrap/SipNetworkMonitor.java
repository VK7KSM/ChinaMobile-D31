...
final class SipNetworkMonitor {
    ...
    static void onConnectivityBroadcast(Context context, Intent source) {
        if (source.getBooleanExtra(ConnectivityManager.EXTRA_NO_CONNECTIVITY, false)) {
            preferences(context).edit()
                    ...
                    .apply();
            ProbeLog.append(context, "网络已完全断开，清除SIP网络指纹");
        }
    ...
    static void onVendorNetworkBroadcast(Context context, Intent source) {
        ...
        ProbeLog.append(context, "原厂已发送有效网络事件，等待记录活动网络指纹：类型="
    ...
    static void handleSettled(Context context, int retryCount) {
        ...
        if (snapshot == null) {
            ...
            ProbeLog.append(context, "SIP网络尚未收敛，延后复核，次数=" + retryCount);
            return;
        }
        ...
        if (isCallBusy(context)) {
            ...
            ProbeLog.append(context, "当前处于响铃或通话状态，延后SIP网络切换，次数=" + retryCount);
            return;
        }
        ...
        if (networkChanged) {
            if (vendorEventCovered) {
                ProbeLog.append(context, "原厂已通知当前网络，跳过重复SIP网络广播："
                ...
                ProbeLog.append(context, "已通知Nexui按新网络重新注册SIP："
                        + snapshot.fingerprint);
