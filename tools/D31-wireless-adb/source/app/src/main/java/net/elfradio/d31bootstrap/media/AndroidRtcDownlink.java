...
public final class AndroidRtcDownlink implements AutoCloseable {
    ...
    private final AppMediaPcmStats pcm=new AppMediaPcmStats(48000);
    ...
    private final Object playoutLock=new Object(),trackCallbacks=new Object();
    private int callbacksInFlight;
    private volatile boolean cancelMuteFailed;
    ...
    private volatile JSONObject subscriptionShape=new JSONObject();
    ...
    public void open()throws Exception {
        ...
        module=JavaAudioDeviceModule.builder(context).setEnableVolumeLogger(false)
                ...
                .setPlaybackSamplesReadyCallback(samples->{
                    pcm.accept(samples.getData(),samples.getAudioFormat(),samples.getChannelCount(),samples.getSampleRate(),unmuted&&!closed);
                    if(!closed&&!playbackFrames&&playoutRequested&&samples.getData().length>0){
        ...
        synchronized(playoutLock){softwareMute(true);}
        ...
        pc=factory.createPeerConnection(config,new PeerConnection.Observer(){
            ...
            public void onTrack(RtpTransceiver transceiver){
                ...
                }catch(Exception|LinkageError failure){fail("MEDIA_DOWNLINK_CALLBACK_FAILED");}
                finally{leaveTrackCallback();}
            }
    ...
    public JSONObject subscribe(JSONObject result)throws Exception{
        ...
        subscriptionShape=DownlinkTrackBinding.shape(result);
    ...
    public void enablePlayout(AudioGuard ownedOutput)throws Exception{
        ...
        ownedOutput.requireIdle();
        synchronized(playoutLock){cancellation.check();softwareMute(false);unmuted=true;}
    }
    ...
    public JSONObject snapshot()throws Exception{return new JSONObject().put("mode","ptt").put("subscribed",acknowledged)
            ...
            .put("subscription_shape",subscriptionShape).put("pcm",pcm.snapshot()).put("cancel_mute_failed",cancelMuteFailed);}
    /** 只更新已核验库的Java软件静音位，不执行带日志回调的setter或JNI；物理清理由close负责。 */
    public void cancel(){
        synchronized(playoutLock){
            closed=true;unmuted=false;cancellation.cancel();
            if(module!=null)try{softwareMute(true);}catch(Exception|LinkageError failure){cancelMuteFailed=true;}
        }
    }
    private void softwareMute(boolean value)throws Exception{
        JavaAudioDeviceModule current=module;
        if(current==null)throw new IOException("MEDIA_PTT_SOFTWARE_MUTE_UNAVAILABLE");
        Object output=JavaAudioDeviceModule.class.getField("audioOutput").get(current);
        if(output==null)throw new IOException("MEDIA_PTT_SOFTWARE_MUTE_UNAVAILABLE");
        java.lang.reflect.Field flag=output.getClass().getDeclaredField("speakerMute");
        if(flag.getType()!=boolean.class||!java.lang.reflect.Modifier.isVolatile(flag.getModifiers()))
            throw new IOException("MEDIA_PTT_SOFTWARE_MUTE_UNAVAILABLE");
        flag.setAccessible(true);flag.setBoolean(output,value);
    }
    private boolean enterTrackCallback(){
        synchronized(trackCallbacks){if(closed)return false;callbacksInFlight++;return true;}
    }
    private void leaveTrackCallback(){synchronized(trackCallbacks){callbacksInFlight--;trackCallbacks.notifyAll();}}
    private void awaitTrackCallbacks()throws Exception{
        long deadline=System.nanoTime()+java.util.concurrent.TimeUnit.MILLISECONDS.toNanos(1750);
        synchronized(trackCallbacks){
            while(callbacksInFlight>0){
                long remaining=deadline-System.nanoTime();
                if(remaining<=0)throw new IOException("MEDIA_PTT_CALLBACK_RELEASE_UNCONFIRMED");
                try{java.util.concurrent.TimeUnit.NANOSECONDS.timedWait(trackCallbacks,remaining);}
    ...
    public void close()throws Exception{
        ...
        // 不持回调锁执行dispose，避免native等待回调与Java关闭相互等待。
        awaitTrackCallbacks();
