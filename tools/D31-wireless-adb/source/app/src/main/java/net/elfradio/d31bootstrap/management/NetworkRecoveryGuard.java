        ...
        try {
            ...
            while (SystemClock.elapsedRealtime() < until) {
                ...
                try {
                    ...
                    if (fresh(beat, task, instance, boot, platform.hash, deadline, SystemClock.elapsedRealtime())) {
                        ...
                        if (beat.getString("start_time").equals(startTime(stat))
                                && cmd.contains("d31-network-guard") && android.system.Os.stat("/proc/" + pid).st_uid == 0
                                && RemoteRuntimeInventory.mapsArchive(maps, platform.apk.getPath())
                                && startTime(stat).equals(startTime(NetworkAndroidFiles.proc("/proc/" + pid + "/stat", 4096)))) return;
                    }
                ...
                JSONObject result = NetworkRecoveryLoop.run(new NetworkRecoveryLoop.Host() {
                    ...
                    public void heartbeat() throws Exception {
                        NetworkAndroidFiles.write(new File(folder, "heartbeat.json"), new JSONObject().put("task_id", task)
                                .put("instance_id", instance).put("boot_id", boot()).put("apk_sha256", hash)
                                .put("deadline_elapsed", deadline).put("elapsed", elapsed()).put("uid", 0)
                                .put("pid", pid).put("start_time", processStart));
                    }
                    public JSONObject query() throws Exception { return platform.query(task); }
