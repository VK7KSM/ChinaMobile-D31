...
final class FirewallControl {
    ...
    static AdbControl.ActionResult enable(Context context) {
        RuleInputs inputs;
        try {
        ...
        AdbControl.ActionResult root = AdbControl.executeRootSequence(
        ...
        AdbControl.ActionResult cleanup = AdbControl.executeRootSequence(
                "启用失败，清理未完成规则", clearCommands());
        preferences(context).edit().putBoolean(ENABLED, !cleanup.succeeded).apply();
        return new AdbControl.ActionResult(describeInputs(inputs)
                + root.log + cleanup.log, false);
    }

    static AdbControl.ActionResult disable(Context context) {
        AdbControl.ActionResult root = AdbControl.executeRootSequence(
    ...
    static AdbControl.ActionResult refreshIfEnabled(Context context) {
        if (!isEnabled(context)) {
            return new AdbControl.ActionResult("D31本机入站防火墙未启用。\n", true);
        }
    ...
    static AdbControl.ActionResult inspectInputs(Context context) {
        try {
            ...
            return new AdbControl.ActionResult(describeInputs(inputs), valid);
        } catch (Throwable error) {
            return new AdbControl.ActionResult(
                    "无法读取安全规则输入：" + error + "\n", false);
        }
    ...
    static String status(Context context) {
        AdbControl.ActionResult result = AdbControl.executeRoot(
                "D31本机入站防火墙状态",
    ...
    private static AdbControl.ActionResult failOpenIfPreviouslyEnabled(
        ...
        AdbControl.ActionResult clear = AdbControl.executeRootSequence(
