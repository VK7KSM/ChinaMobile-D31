...
/** 解析D31已取证的空闲形状；活动输出另须本次完整身份和严格Policy交叉验证。 */
public final class D31OutputEvidence {
    public enum State { EMPTY, STARTING, SELF_ONLY, BUSY, UNKNOWN }
    ...
    public static Result evaluate(AudioInputOwnership.Dump flinger,AudioInputOwnership.Dump policy,int pid,long now){
        ...
        try{
            ...
            for(int i=0;i<f.size();i++)if(f.get(i).startsWith("Output thread ")){
                ...
                if(total<0||total>64)throw new IllegalArgumentException();
                if(!field(block,"Standby:").equals("yes"))return D31ActiveOutputEvidence.evaluate(flinger.text,policy.text,null);
    ...
    /** 只有本次实际播放器的完整身份才能进入已取证的活动格式分支。 */
    public static Result evaluate(AudioInputOwnership.Dump flinger,AudioInputOwnership.Dump policy,D31OutputIdentity identity,long now){
        if(identity==null)return unknown("MEDIA_OUTPUT_IDENTITY_MISSING");
        Result baseline=evaluate(flinger,policy,identity.pid,now);
        if(!"MEDIA_OUTPUT_ACTIVE_SHAPE_UNVERIFIED".equals(baseline.reason))return baseline;
        return D31ActiveOutputEvidence.evaluate(flinger.text,policy.text,identity);
    }
