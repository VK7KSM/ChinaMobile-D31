...
public class RemoteLocationSamplerTest {
    ...
    /** 无线观测先于GPS进入缓存；期间读报告不会再发起一次扫描。 */
    @Test(timeout = 5000) public void radioArrivesBeforeGpsAndReadingReportStartsNoNewScan() throws Exception {
        ...
        try (RemoteLocationSampler sampler = new RemoteLocationSampler((window, includeRadio) -> {
            ...
            assertTrue(sampler.tick()); assertTrue(entered.await(2, TimeUnit.SECONDS));
            ...
            assertFalse(sampler.tick()); assertEquals(2, reads.get());
    ...
    @Test(timeout = 5000) public void closeInterruptsWorkerAndPreventsNewTicks() throws Exception {
        ...
        assertTrue(exited.await(2, TimeUnit.SECONDS)); assertFalse(sampler.tick());
    }
