...
public class AlarmLoopCountTest {
    private static final int LOOP_MS=AlarmWaveform.loopMs(AlarmWaveform.samples().length);

    @Test public void roundsUpSoPlaybackIsNeverCutShort(){
        // 片段长度固定1.4秒：10秒警报需要8段（11.2秒），不能是7段（9.8秒）。
        assertEquals(1400,LOOP_MS);
        assertEquals(8,AlarmWaveform.loopCount(AlarmTasks.DURATION_MS,LOOP_MS));
        assertEquals(22,AlarmWaveform.loopCount(AndroidAlarm.REPEAT_BURST_MS,LOOP_MS));
        for(int durationMs:new int[]{1,1399,1400,1401,2800,2801,9999,10000,30000,600000}){
            ...
            assertTrue("次数不得使播放早于请求时长结束 durationMs="+durationMs,(long)count*LOOP_MS>=durationMs);
            assertTrue("不得多于一个片段的余量 durationMs="+durationMs,(long)(count-1)*LOOP_MS<durationMs);
        }
    }

    @Test public void alwaysFiniteAndNeverTheInfiniteSentinel(){
        // AudioTrack.setLoopPoints 用 -1 表示无限循环；任何输入都不得返回该值或非正数。
        for(int durationMs:new int[]{Integer.MIN_VALUE,-1,0,1,10000,Integer.MAX_VALUE}){
            ...
            assertTrue("次数必须为有限正数 durationMs="+durationMs,count>=1&&count<Integer.MAX_VALUE);
        }
