...
完整190起，同一原件增加独立 `productAccess`，不改变38项合同。固定八项应用操作和三项辅助服务事实：操作模式保留允许、忽略、拒绝及默认；默认不等于允许。辅助服务分别读取总开关、Guard授权名单及当前绑定反馈列表。只读查询不会发送短信、打开相机、录音或改变辅助服务。

用上述原宿主分别保全两台设备的报告后，可执行：

```text
python -B tools/baseline/compare_product_access.py --left <第一份原报告> --right <第二份原报告> --factory-source <已冻结FactoryInit.java> --output <全新比较目录>
```

比较器严格校验11项合同，绑定正式初始化源码摘要，只对有明确消费者依据的四项提供初装参考。两台一致不证明两台都正确；未知和读取失败保留。辅助反馈列表未命中不能单独判为服务崩溃，自动化服务可能抑制该列表。工具不验证原远程任务身份，须连同宿主保全记录使用，不生成或执行修复。
