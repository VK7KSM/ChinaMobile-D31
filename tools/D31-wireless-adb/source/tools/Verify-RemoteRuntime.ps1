...
$script:sequence=0
function Read-Device([string]$Command,[int]$TimeoutMilliseconds=20000){
    $script:sequence++
...
function Get-RuntimeServiceState([string]$Text){
    $records=[regex]::Matches($Text,'ServiceRecord\{')
    $empty=$Text -cmatch '(?m)^\s*\(nothing\)\s*$'
    if($records.Count -eq 0 -and $empty){return 'EMPTY'}
    if($records.Count -eq 1 -and -not $empty -and $Text -cmatch '(?m)^\s*\* ServiceRecord\{[a-f0-9]+ u0 net\.elfradio\.d31bootstrap/(?:\.telemetry|net\.elfradio\.d31bootstrap\.telemetry)\.LocationCacheService\}\s*$'){
        return 'LOCATION_ONLY'
    }
    return 'OTHER_OR_UNKNOWN'
}
function Wait-LocationIdle {
    $locationWatch=[Diagnostics.Stopwatch]::StartNew()
    $wait=[ordered]@{budgetMs=60000;elapsedMs=0;polls=0;state='CHECKING';waited=$false}
    function Read-WithinLocationWait([string]$Command){
        $remaining=60000-$locationWatch.ElapsedMilliseconds
        if($remaining -le 0){throw '定位服务自然退出等待超时'}
        $value=Read-Device $Command ([int][Math]::Min(20000,$remaining))
        if($locationWatch.ElapsedMilliseconds -ge 60000){throw '定位服务自然退出等待超时'}
        return $value
    }
    try {
        while($true){
            $services=Read-WithinLocationWait 'dumpsys activity services net.elfradio.d31bootstrap'
            $wait.polls++
            $state=Get-RuntimeServiceState $services
            if($state -ceq 'EMPTY'){$wait.state='EMPTY';break}
            if($state -cne 'LOCATION_ONLY'){throw '服务不是唯一已知按需定位服务，禁止等待'}
            $wait.waited=$true
            $locationPid=Read-WithinLocationWait 'busybox pidof net.elfradio.d31bootstrap; code=$?; test "$code" -eq 0 -o "$code" -eq 1'
            if($locationPid -notmatch '^[1-9][0-9]*$'){throw '定位等待期间APP进程不是唯一实例'}
            $threads=Read-WithinLocationWait ('for f in /proc/'+$locationPid+'/task/*/comm; do cat "$f" || exit 1; done')
            if($threads -match '(?m)^d31-(audio-|app-media|local-audio|rtc-|video-|visual-|on-demand-|alarm-|ptt-)'){
                throw '定位等待期间发现媒体线程，禁止等待'
            }
            $remaining=60000-$locationWatch.ElapsedMilliseconds
            if($remaining -le 0){throw '定位服务自然退出等待超时'}
        ...
        $wait.state='REJECTED'; throw
    } finally {
        $wait.elapsedMs=$locationWatch.ElapsedMilliseconds
        $wait | ConvertTo-Json | Out-File "$capture/location-wait.json" -NoClobber
    }
}
...
# 仅等待已知定位服务自然退出；随后从boot、活动版本和健康开始全新验收。
Wait-LocationIdle
...
$result=[ordered]@{
    ...
    servicesEmpty=((Get-RuntimeServiceState $services) -ceq 'EMPTY')
    mediaThreadsPresent=($appThreads -match '(?m)^d31-(audio-|app-media|local-audio|rtc-|video-|visual-|on-demand-|alarm-|ptt-)')
