...
class ConversationsAdapter @Inject constructor(
    ...
    override fun onBindViewHolder(holder: QkBindingViewHolder<ConversationListItemBinding>, position: Int) {
        ...
        bindConversation(holder, conversation)
    }

    fun bindConversation(holder: QkBindingViewHolder<ConversationListItemBinding>, conversation: Conversation) {
        val binding = holder.binding
        binding.root.setBackgroundResource(R.drawable.d31_conversation_sim)
        binding.sipAvatar.isVisible = false
        binding.avatars.isVisible = true
        binding.root.setOnClickListener {
            if (toggleSelection(conversation.id, false)) binding.root.isActivated = isSelected(conversation.id)
            else navigator.showConversation(conversation.id)
        }
        ...
        binding.date.text = "SIM · " + (conversation.date.takeIf { it > 0 }?.let(dateFormatter::getConversationTimestamp) ?: "")
