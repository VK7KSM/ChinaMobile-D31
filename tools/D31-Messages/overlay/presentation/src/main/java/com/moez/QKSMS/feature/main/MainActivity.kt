...
import dev.octoshrimpy.quik.feature.phone.UnifiedConversationsAdapter
...
class MainActivity : QkThemedActivity(), MainView {
    ...
    private lateinit var unifiedAdapter: UnifiedConversationsAdapter
...
//    override val plusBannerIntent by lazy { plusBanner.clicks() }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        unifiedAdapter = UnifiedConversationsAdapter(this, conversationsAdapter) { empty ->
            if (binding.recyclerView.adapter === unifiedAdapter) binding.empty.isVisible = empty
        }
        binding.composeSip.setOnClickListener { UnifiedConversationsAdapter.openSip(this) }
        androidx.appcompat.widget.TooltipCompat.setTooltipText(binding.compose, "新建SIM短信")
        androidx.appcompat.widget.TooltipCompat.setTooltipText(binding.composeSip, "新建SIP短信")
        ...
        itemTouchCallback.adapter = unifiedAdapter
        ...
        theme
                ...
                .subscribe { theme ->
                    ...
                    binding.compose.setBackgroundTint(android.graphics.Color.rgb(30, 136, 229))

                    // Set the FAB compose icon color
                    binding.compose.setTint(android.graphics.Color.WHITE)
                }
    }
...
//        plus.isVisible = state.upgraded
        ...
        binding.composeSip.setVisible(state.page is Inbox || state.page is Archived)
        binding.composeSimLabel.isVisible = binding.compose.isVisible
        binding.composeSipLabel.isVisible = binding.composeSip.isVisible
        ...
        when (state.page) {
            is Inbox -> {
                ...
                if (binding.recyclerView.adapter !== unifiedAdapter)
                    binding.recyclerView.adapter = unifiedAdapter
                conversationsAdapter.updateData(state.page.data)
                unifiedAdapter.showInbox()
            ...
            is Searching -> {
                showBackButton(true)
                if (binding.recyclerView.adapter !== unifiedAdapter) binding.recyclerView.adapter = unifiedAdapter
                unifiedAdapter.showSearch(state.page.data ?: listOf(), binding.toolbarSearch.text.toString())
    ...
    override fun onResume() =
        super.onResume().also { activityResumedIntent.onNext(true); unifiedAdapter.refreshSip() }
