...
import dev.octoshrimpy.quik.feature.phone.D31MessageNotifications
...
class NotificationManagerImpl @Inject constructor(
    ...
    private val d31: D31MessageNotifications
) : dev.octoshrimpy.quik.manager.NotificationManager {

    companion object {
        const val DEFAULT_CHANNEL_ID = D31MessageNotifications.MESSAGE_CHANNEL
     ...
     */
    override fun update(threadId: Long) = updateMessages(threadId, false)

    override fun notifyIncoming(threadId: Long) = updateMessages(threadId, true)

    @Synchronized private fun updateMessages(threadId: Long, incoming: Boolean) {
        if (activeConversationManager.getActiveConversation() == threadId) {
            messageRepo.markRead(listOf(threadId))
            notificationManager.cancel(threadId.toInt())
            notificationManager.cancel(threadId.toInt() + 100000)
            return
        }
        val messages = messageRepo.getUnreadUnseenMessages(threadId).filter { !it.isMe() }
        if (messages.isEmpty()) {
            notificationManager.cancel(threadId.toInt())
            notificationManager.cancel(threadId.toInt() + 100000)
            return
        ...
        val fresh = d31.isNew("sim:$threadId", latestId, incoming)
        val contentIntent = D31MessageNotifications.conversationIntent(context, "sim", threadId.toString(), latestId)
        ...
        val ringtone = d31.sound(prefs.ringtone(threadId).get())
        val channelId = d31.channel(getChannelIdForNotification(threadId))
        ...
        val quiet = prefs.silentNotContact.get() && run {
    ...
    // Tell the notification if it's a group message
                    ...
                    when (action) {
                        ...
                        Preferences.NOTIFICATION_ACTION_REPLY -> {
                            if (Build.VERSION.SDK_INT >= 24) {
                                ...
                                val intent = D31MessageNotifications.conversationIntent(context, "sim", threadId.toString(), latestId)
        ...
        // 联系人图片和快捷方式加载期间可能切入会话，发布前再次核对。
        if (activeConversationManager.getActiveConversation() == threadId) {
            messageRepo.markRead(listOf(threadId))
            notificationManager.cancel(threadId.toInt())
            return
        }
        val alert = d31.apply(notification, fresh, threadId, quiet, channelId)
        notificationManager.notify(threadId.toInt(), notification.build())
        d31.posted("sim:$threadId", latestId)
        d31.wake(alert, channelId, threadId)
    }
    ...
    override fun notifyFailed(msgId: Long) {
        ...
        val notification = NotificationCompat.Builder(context, getChannelIdForNotification(threadId))
                ...
                .setAutoCancel(true)
                .setContentIntent(contentPI)
                .setSound(d31.sound(prefs.ringtone(threadId).get()))
                .setLights(Color.WHITE, 500, 2000)
                .setVibrate(if (prefs.vibration(threadId).get()) VIBRATE_PATTERN else longArrayOf(0))

        if (d31.inCall() || prefs.ringtone(threadId).get().isEmpty()) notification.setSilent(true)
        ...
        val channels: List<NotificationChannel> = when (threadId) {
            0L -> listOf(
                NotificationChannel(
                    ...
                    setSound(d31.sound(prefs.ringtone().get()), AudioAttributes.Builder()
                ...
                listOf(
                    NotificationChannel(channelId, title, NotificationManager.IMPORTANCE_HIGH).apply {
                        ...
                        setSound(d31.sound(prefs.ringtone(threadId).get()), AudioAttributes.Builder()
