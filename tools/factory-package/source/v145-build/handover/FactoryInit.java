...
public final class FactoryInit {
    ...
    static void locationDefaults(boolean apply) throws Exception {
        if(apply) {
            putSetting("secure","location_mode","3");
            putSetting("secure","location_providers_allowed","+gps");
            putSetting("secure","location_providers_allowed","+network");
        }
        Set<String> providers=new HashSet<>(Arrays.asList(setting("secure","location_providers_allowed").split(",")));
        String mode=setting("secure","location_mode");
        if(!(mode.equals("3") || mode.equals("null") || mode.isEmpty())
                || !providers.contains("gps") || !providers.contains("network"))
            throw new IOException("Factory location settings mismatch");
    }
