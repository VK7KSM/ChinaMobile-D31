...
此目录当前安装器为1.4.6，原始固件、私人日志、用户数据和签名私钥不在源码目录内。下方1.4.5及更早章节保留为历史说明，不代表当前默认批准合同。

## 1.4.6完整刷机

`source/build_v146.py`从已发布且摘要固定的1.4.5签名包派生新包，使用`D31_PACKAGE_146`编译安装器；除安装器、清单和签名外，所有载荷逐项保持相同。`source/native/test_native_146.c`及`run_native_146.sh`执行生产C流程的离线故障注入。

`source/approved-package-v1.4.6.json`和默认批准合同明确`replaceBootRecovery=true`。新流程写入配套boot和Recovery，清除旧用户数据，保留本机身份和校准；不再拒绝内容不同的旧boot/Recovery。写入顺序和验收范围见[完整刷机说明](../../docs/D31-v1.4.6完整刷机说明.md)。源码保留144、145编译分支，仅供历史兼容性回归。
