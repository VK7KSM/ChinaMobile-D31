...
**uptool恢复的是管理连接，不会自动修复所有启动故障。** 本项目D31随后已恢复启动，并完成8765探针开机验收；早期故障记录中的“尚未恢复”是当时状态。不要用反复重刷代替故障检查。
...
原厂`uptool`使用`EtherType 0x9974`原始以太网帧，不是TCP/UDP端口。内核、有线链路及服务仍工作时，即使桌面和ADB不可用，也可能恢复管理；无法保证解决引导器或内核损坏。

[完整指令、使用方法与防护说明](docs/D31-uptool指令与安全说明.md)列出逆向识别的全部18个动作码，以及其他已识别原厂入口，明确区分实测与静态发现。实际操作先单播查询目标，再使用固定ADB恢复脚本；不要直接试升级或恢复出厂动作。

**本机样本允许无管理凭据的root命令执行，同一二层网络中的恶意设备可能滥用。** MAC及MD5校验不是认证，关闭5555/8765或普通IP端口防火墙不能据此证明堵住uptool。维修时使用可信、隔离的二层网络；日常可评估外部二层过滤或按需停用，但应先验证备用救援。当前没有因此关闭服务或新增管理限制。
