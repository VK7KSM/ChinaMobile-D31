...
6. 下载新版首次引导工具[D31-wireless-adb-v1.11.0.apk](tools/D31-wireless-adb/D31-wireless-adb-v1.11.0.apk)。在Windows中运行`fsquirt`，或者右键点击任务栏蓝牙图标选择“发送文件”，把这个APK发送给D31。它是原`v1.0`的同应用升级版；旧Windows工具包内附带的APK本轮未更新，请使用这里的单独下载。
...
**当前已验证的是恢复ADB管理连接，不是整机修复完成。** 隔离异常启动脚本后的整机启动仍在排查，不承诺一次执行就恢复桌面。现有启动补丁正在故障复核，暂不建议用当前刷机包反复重刷来尝试解决此故障。

### 方法一：卡开机、ADB连不上，先恢复管理连接

1. 保持D31通电，把D31和Windows电脑都接入同一个有线局域网。需要同一二层网络，不能只通过互联网、VPN或跨路由器连接；这一步不需要互联网访问D31。
2. 电脑安装[官方Npcap](https://npcap.com/#download)、[Python 3](https://www.python.org/downloads/windows/)和[Android平台工具](https://developer.android.com/tools/releases/platform-tools)。Npcap安装会请求管理员权限，可能短暂影响电脑网络。
3. 下载[本项目代码ZIP](https://github.com/VK7KSM/ChinaMobile-D31/archive/refs/heads/main.zip)并解压，打开其中的`tools/recovery`目录。里面的两个脚本分别用于查询设备和临时恢复ADB，不是Windows刷机工具源码。
4. 按[以太网救援详细步骤](docs/D31以太网救援.md)核实电脑有线网卡、D31有线MAC和IP，在管理员PowerShell中先运行`query_uptool.py`。只有收到`action=0x0102`且`checksum_valid=true`的目标回复，才继续运行`restore_adbd_uptool.py`。
5. 恢复脚本只向指定D31发送一次固定命令：检查root身份，将临时ADB端口设为5555，再启动adbd。**不刷分区、不清数据、不关闭认证、不修改防火墙、不重启整机。** `request_sent=true`只代表发送成功，不代表ADB已恢复；脚本没有直接响应也不等于执行失败。
6. 在同一PowerShell中，用自己的D31有线IP连接。下面的`$adb`和`$ip`按详细步骤设置；电脑ADB服务端口是5042，设备端口是5555，两者不是一回事。
...
7. 收到shell结果后，先保存日志和启动脚本，再按实际故障修复。`sys.boot_completed=1`只是启动完成的一项指标，还要检查桌面和应用。不要直接点击“开始刷机”，因为完整刷机流程会清空数据。

**没有回复或仍连不上时：**保留脚本输出目录，核对有线网卡和MAC，不要不断重复发送或杀掉电脑全部ADB进程。原厂服务未启动、设备防火墙、ADB认证或启动程序再次关闭adbd，都可能使此方法不能完成连接。脚本使用固定会话号，原厂服务可能去重同一会话；不能把反复运行当作可靠重试。详细步骤也列出了限制和后续取证方法。

### 方法二：能进入Recovery，且有这台机器的刷机前备份

**这条路线可能清除用户数据。** 当前尚未验证中国移动定制D31通用的物理按键进入Recovery方法，也未完成本机急救包的真机卡刷验收。不要把“复制文件到卡里”理解为开机会自动升级。下面步骤仅适用于已经实际进入Recovery、并确认菜单支持相应介质的机器。

1. 先确认刷机时是否保留了默认备份选项。如果当时取消了备份，电脑中没有本机急救包，不能使用这条备份恢复路线；仍可尝试上面的以太网救援。不要使用另一台D31的急救包。
2. 在电脑的`D31备份`中找到这台D31的急救包，把其中名为`需要抢救时复制到TF卡或U盘`的目录及全部文件原样复制到FAT32格式的TF卡或U盘。
3. 如果ADB已经恢复且准备执行本机备份恢复，可以使用`& $adb -P 5042 -s "${ip}:5555" reboot recovery`请求进入Recovery。这是Android标准命令，本次故障恢复中没有执行或验收该入口；没有看到实际Recovery菜单就不能继续卡刷。
4. 给D31接上稳定电源并插入FAT32介质。在实际菜单中选择`Apply update from SD card`或确实存在的USB介质入口；不要假定每个批次都有USB OTG菜单。
...
没有进入Recovery，卡里的ZIP就没有执行入口；这**不能直接证明**Recovery或硬件已经损坏。鼠标能动、键盘能亮，也不代表“系统正在启动”窗口允许操作，Android启动窗口可能吞掉输入。不要盲猜组合键、短接接口，或用普通双USB-A线连接电脑和D31两个主机口。

先保留已有日志，再取得匹配该批次的厂商维修入口与固件。当前Windows工具没有经验证的BootROM底层救砖路线。不要把其他D31的`nvram`、`nvdata`、`proinfo`或校准分区写入故障机。此次以太网接口的[实测记录和限制](docs/D31以太网救援.md)会随恢复进度更新。
